  ...
  event.respondWith(
    caches.open(CACHE_NAME).then(function (cache) {
      return cache.match(event.request).then(function (cachedResponse) {
        // B8. If the request is in the cache, return it
        if (cachedResponse) {
          return cachedResponse;
        }

        // B8. Otherwise fetch the resource from the network
        return fetch(event.request).then(function (networkResponse) {
          // Only cache valid responses (e.g., status is 200, type is basic)
          if (
            networkResponse &&
          ...
          // Optionally handle fetch failure (e.g., offline and not cached)
